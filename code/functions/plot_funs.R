...
raincloud_plot <- function(dat, title, xlab, ylab,
  ...
  if(include_grouping == FALSE){
    rain_plot <- ggplot(data = dat, 
      ...
      geom_point(aes_string(y = outcome_var, color = predictor_var), position = position_jitter(width = 0.15), 
                 size = 0.75, alpha = 0.6) +
    ...
    rain_plot <- ggplot(data = dat, 
      ...
      geom_flat_violin(position = position_nudge(x = 0.2, y = 0), alpha = 0.6) +
      geom_point(aes_string(y = outcome_var, color = group_var), position = position_jitter(width = 0.15), 
                 size = 0.75, alpha = 0.6) +
      geom_boxplot(width = 0.1, outlier.shape = NA, alpha = 0.5) + 
