...
// Priors
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {
      sv_a   = (1 * amount_a[i, t])  - (kE[i] * (effort_a[i, t]^2));
      sv_b   = (1 * amount_b[i, t])  - (kE[i] * (effort_b[i, t]^2));
      choice[i, t] ~ bernoulli_logit(b[i] * (a[i] + (sv_a - sv_b)));
    }
  }
}
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      for (t in 1:(Tsubj[i])) {
        sv_a   = (1 * amount_a[i, t])  - (kE[i] * (effort_a[i, t]^2));
        sv_b   = (1 * amount_b[i, t])  - (kE[i] * (effort_b[i, t]^2));
        log_lik[i] += bernoulli_logit_lpmf(choice[i, t] | b[i] * (a[i] + (sv_a - sv_b)));

        // generate posterior prediction for current trial
        y_pred[i, t] = bernoulli_rng(inv_logit(b[i] * (a[i] + (sv_a - sv_b))));
      }
    }
  }
}
